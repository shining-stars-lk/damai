...
local program_seat_no_sold_hash_key = KEYS[3]
local increase_data_list = cjson.decode(ARGV[1])
local un_lock_seat_id_list = cjson.decode(ARGV[2])
local seat_no_sold_data_list = cjson.decode(ARGV[3])
-- 恢复库存
