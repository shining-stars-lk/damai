...
## key的约定说明
**提供的api中，key的类型为`RedisKeyWrap`,需要使用提供的方法来转换**
1. 将key存放在`RedisKeyEnum`枚举中
```java
/**
 * @program: redis-tool
...
```
2. 使用`RedisKeyWrap`的`cacheKeyBuild`方法传入`RedisKeyEnum`来构建`RedisKeyWrap`。
...
public Object test(TestCacheDto dto) {
        ...
        distributCache.set(RedisKeyWrap.cacheKeyBuild(RedisKeyEnum.test_key,hospitalNo),dto);
        
        TestCacheDto testDto = distributCache.get(RedisKeyWrap.cacheKeyBuild(RedisKeyEnum.test_key,hospitalNo),
        TestCacheDto.class);
        return testDto;
}
```
