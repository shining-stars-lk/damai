...
<dependency>
    ...
    <artifactId>distribute-cache</artifactId>
...
```
**此工具被`spring`管理，所以用常规的方式从`spring`获取即可.**

接口名:`DistributCache` </br>
redis实现类名:`RedisCacheImpl`
...
## key的约定说明
**提供的api中，key的类型为`CacheKeyWrap`,需要使用提供的方法来转换**
...
```
2. 使用`CacheKeyWrap`的`cacheKeyBuild`方法传入`CacheKeyEnum`来构建`CacheKeyWrap`。
   此方法支持`String.format`占用符的使用
```java
/**
 * 构建真实的key
...
public Object test(TestCacheDto dto) {
        String hospitalNo = dto.getHospitalNo();
        
        distributCache.set(CacheKeyWrap.cacheKeyBuild(CacheKeyEnum.test_key,hospitalNo),dto);
        
        TestCacheDto testDto = distributCache.get(CacheKeyWrap.cacheKeyBuild(CacheKeyEnum.test_key,hospitalNo),
        TestCacheDto.class);
        return testDto;
}
```
